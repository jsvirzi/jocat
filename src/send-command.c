...
int main(int argc, char **argv) {
    ...
    sendto(sockfd, command, command_len,MSG_CONFIRM, (const struct sockaddr *) &servaddr, sizeof(servaddr));
    ...
    while (now < end_time) {
        ...
        if (flag) {
            int n = recvfrom(sockfd, &buffer[rx_pos], rx_len - rx_pos,MSG_WAITALL, (struct sockaddr *) &servaddr, &len);
            if (n > 0) { rx_pos += n; }
        }
        now = elapsed_time();
    }
